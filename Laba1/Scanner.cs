...
namespace Laba1
{
    ...
    */
    public enum TokenType
    {
        N, V, A, Pro, PropN, Det, P, Unknown, EOF
    }
    ...
    public class Lexer
    {
        private static HashSet<string> Nouns = new HashSet<string> { "flight", "passenger", "trip", "morning" };
        private static HashSet<string> Verbs = new HashSet<string> { "is", "prefers", "like", "need", "depend", "fly" };
        private static HashSet<string> Adjectives = new HashSet<string> { "cheapest", "non-stop", "first", "latest", "other", "direct" };
        private static HashSet<string> Pronouns = new HashSet<string> { "me", "I", "you", "it" };
        private static HashSet<string> PropNouns = new HashSet<string> { "Alaska", "Baltimore", "Los Angeles", "Chicago" };
        private static HashSet<string> Determiners = new HashSet<string> { "the", "a", "an", "this", "these", "that" };
        private static HashSet<string> Prepositions = new HashSet<string> { "from", "to", "on", "near" };

        private RichTextBox errorBox;

        public Lexer(RichTextBox errorBox)
        {
        ...
        public List<Token> Tokenize(string input)
        {
            ...
            foreach (var word in words)
            {
                ...
                if (Nouns.Contains(w))
                    tokens.Add(new Token { Type = TokenType.N, Lexeme = w });
                else if (Verbs.Contains(w))
                    tokens.Add(new Token { Type = TokenType.V, Lexeme = w });
                else if (Adjectives.Contains(w))
                    tokens.Add(new Token { Type = TokenType.A, Lexeme = w });
                else if (Pronouns.Contains(w))
                    tokens.Add(new Token { Type = TokenType.Pro, Lexeme = w });
                else if (PropNouns.Contains(w))
                    tokens.Add(new Token { Type = TokenType.PropN, Lexeme = w });
                else if (Determiners.Contains(w))
                    tokens.Add(new Token { Type = TokenType.Det, Lexeme = w });
                else if (Prepositions.Contains(w))
                    tokens.Add(new Token { Type = TokenType.P, Lexeme = w });
                else
                {
                    errorBox.AppendText($"[Lexer Error] Unknown token: '{w}'\r\n");
                    // Пропускаем неизвестный токен
                }
            }

            tokens.Add(new Token { Type = TokenType.EOF, Lexeme = "<EOF>" });
            return tokens;
        }
